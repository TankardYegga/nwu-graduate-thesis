...
class input_data(object):
	def __init__(self, args):
		...
		#paper neighbor: author + citation + venue ，形如a0,a1,p1,p3,v4
				...
				if len(self.a_p_list_train[j]):
					...
					for l in range(self.args.walk_L - 1):
						if curNode[0] == "a":
							...
							curNode = int(curNode[1:])
                            # 如果当前节点是paper节点，则根据前一个结点来决定后一个结点的类型
							# 如果前面一个节点的类型是author节点，则后面的节点仍然是p结点
							# 不过节点编号是paper结点对应的venue结点编号
							# 否则是其邻居结点的任意类型
							if preNode[0] == "a":
	...
	def a_p_citation_train_test(self):
		...
		# t=0表示作者引用论文的训练集，t=1表示作者引用论文的测试集
