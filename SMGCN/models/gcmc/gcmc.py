...
class GCMC(BASE_GNN):
    def __init__(self, **kwargs):
        super(GCMC, self).__init__(**kwargs)

        self.adj_type = self.args.adj_type
        self.alg_type = self.args.alg_type
    ...
    def _init_weights(self):
        ...
        for k in range(self.n_layers):
            ...
            all_weights['W_mlp_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k+1], self.weight_size_list[k+1]]), name='W_mlp_%d' % k)
            all_weights['b_mlp_%d' % k] = tf.Variable(
                initializer([1, self.weight_size_list[k+1]]), name='b_mlp_%d' % k)

        # concated_size = sum(self.weight_size_list)
        # print('concated size is', concated_size)
        # # MLP层卷积层的参数初始化
        ...
        # )
        # all_weights['MLP_W'] = tf.Variable(
        #     initializer([ self.weight_size_list[k+1],  self.weight_size_list[k+1]]),
        #     name='MLP_W'
        # )
        # all_weights['MLP_b'] = tf.Variable(
    ...
    def _create_gcmc_embed(self):
        ...
        embeddings = tf.concat([self.weights['sympt_embedding'], self.weights['herb_embedding']], axis=0)
    ...
    def _train(self):
        self.sympt_embeddings, self.herb_embeddings = self._create_gcmc_embed()
