...
使得sge的卷积层也可以是多层的
"""
class BiparGCN_W_SGE(BASE_GNN):
    def __init__(self, **kwargs):
        self.model_type='smgcn'
        super(BiparGCN_W_SGE, self).__init__(**kwargs)
        self._train()
    ...
    def _init_weights(self):
        ...
        # 协同图卷积层的参数初始化
        self.sge_num_layers = int(self.args.sge_num_layers)
        for k in range(self.sge_num_layers):
            original_input_dim = self.initial_embed_size if k == 0 else self.weight_size_list[-1]
            all_weights['V_symp_%d' % k] = tf.Variable(
                initializer([original_input_dim, self.weight_size_list[-1]]),
                name = 'V_symp_%d' % k
            )
            all_weights['B_symp_%d' % k] = tf.Variable(
                initializer([1, self.weight_size_list[-1]]),
                name = 'B_symp_%d' % k
            )
            all_weights['V_herb_%d' % k] = tf.Variable(
                initializer([ original_input_dim, self.weight_size_list[-1]]),
                name = 'V_herb_%d' % k
            )
            all_weights['B_herb_%d' % k] = tf.Variable(
                initializer([1, self.weight_size_list[-1]]),
                name = 'B_herb_%d' % k
            )
    ...
    def _create_smgcn_embed(self):
        ...
        # 首先获得症状的嵌入矩阵
        sympt_synergy_embed_mat = self.weights['sympt_embeddings']
        herb_synergy_embed_mat = self.weights['herb_embeddings']
        
        for k in range(self.sge_num_layers):
            sympt_synergy_embed_mat = tf.nn.tanh(tf.matmul( tf.matmul(sympt_sympt_adj_mat,
                                                                    sympt_synergy_embed_mat), self.weights['V_symp_%d' % k])  +
                                                self.weights['B_symp_%d' % k]      )
            # 再获得草药的嵌入矩阵
            herb_synergy_embed_mat = tf.nn.tanh(tf.matmul(tf.matmul(herb_herb_adj_mat,
                                                                    herb_synergy_embed_mat),self.weights['V_herb_%d' % k]) +
                                                self.weights['B_herb_%d' % k])
        # 将两个嵌入矩阵堆在一起
