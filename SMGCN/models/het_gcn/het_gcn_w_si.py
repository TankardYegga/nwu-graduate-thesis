...
class HetGCN_W_SI(BASE_GNN):
    def __init__(self, **kwargs):
        self.model_type='hetgcn'
        self.sess = tf.InteractiveSession()
        super(HetGCN_W_SI, self).__init__(**kwargs)
        self._train()
    ...
    def _init_weights(self):
        ...
        for k in range(self.n_layers):
            ...
            all_weights['weight_attention_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k] + self.weight_size_list[k+1], self.weight_size_list[k+1]]),
                name = 'weight_attention_%d' % k,
            )
            all_weights['z_attention_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k+1],1]),
