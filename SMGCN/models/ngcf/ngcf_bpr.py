...
class NGCF_BPR(BASE_GNN):

    def __init__(self, **kwargs):
        super(NGCF_BPR, self).__init__(**kwargs)
    ...
    def _inference(self):
        ...
        self.mean_sympt_set_embeddings = tf.matmul(self.sympt_set_normalized_mat, self.sympt_set_embeddings_sum)

        # # 将获得的平均症状嵌入通过MLP获得一个综合的综合症嵌入表示
        # self.syndrome_embedding = tf.nn.relu(tf.matmul(self.mean_sympt_set_embeddings, self.weights['MLP_W']) + \
        #                                      self.weights['MLP_b'])
        self.syndrome_embedding = self.mean_sympt_set_embeddings
